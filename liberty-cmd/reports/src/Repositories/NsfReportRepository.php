...
class NsfReportRepository extends SqlSrvRepository
{
    /**
     * Retrieve all NSF reports for CSV export.
     *
    ...
    public function all(
        ...
        return $this->baseQuery($nsfFrom, $nsfTo, $submittedFrom, $submittedTo, $filters)->get();
    }

    /**
     * Retrieve paginated NSF reports for the UI.
     *
     * @param  array<string, mixed>  $filters
     */
    public function paginate(
        ...
        return $this->paginateBuilder(
            $this->baseQuery($nsfFrom, $nsfTo, $submittedFrom, $submittedTo, $filters),
            $perPage
        );
    }

    /**
     * Gather select options for the NSF report filters.
     *
    ...
    /**
     * Build the shared NSP report query.
     *
     * @param  array<string, mixed>  $filters
     */
    protected function baseQuery(
        ...
        if ($nsfFrom) {
            $query->whereDate('NSF_Date', '>=', $nsfFrom);
        }

        if ($nsfTo) {
            $query->whereDate('NSF_Date', '<=', $nsfTo);
        }
    ...
    /**
     * Apply user supplied filters to the NSF query.
     *
