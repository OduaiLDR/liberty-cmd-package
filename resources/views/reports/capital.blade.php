...
@section('content')
    @php
        ...
        $isCurrencyKey = static function (string $key): bool {
            $k = strtolower($key);
            if (str_contains($k, 'date')) return false;
            if ($k === 'tranche') return false;
            return (bool) preg_match('/(amount|payment|debt|epf)/', $k);
        };
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center py-0">
            <h6 class="py-3 mb-0">Capital Report</h6>
        </div>
        <div class="card-body">
            <form method="get" action="{{ route('cmd.reports.capital_report') }}" class="bg-light rounded-3 p-2 p-md-3 mb-3" id="capital-report-form">
                ...
                <div class="row g-2 align-items-center">
                    <div class="cg-2 align-items-center justify-content-end">
                        <span class="fw-semibold">Capital Report For :</span>
                    </div>
                ...
                <details class="mt-2 mb-2">
                    ...
                    <div class="row g-2 mt-2">
                        <div class="col-md-3">
                            <label class="form-label form-label-sm">Tranche (contains)</label>
                ...
                <div class="col-auto">
                    <div class="d-flex gap-2 justify-content-end">
                        ...
                        <a href="{{ route('cmd.reports.capital_report') }}" class="btn btn-light btn-sm">Reset</a>
                        <div class="col-auto">
            ...
            @php
                // Calculate summary totals for currency columns
                $summaryTotals = [];
                foreach (array_keys($columns) as $key) {
                    if ($isCurrencyKey($key)) {
                        $summaryTotals[$key] = 0;
                        foreach ($reports as $r) {
                            $summaryTotals[$key] += (float) ($r->{$key} ?? 0);
                        }
                    }
                }
            @endphp
            ...
            <div class="alert alert-warning mb-3">
                <div class="row">
                    ...
                    @foreach ($summaryTotals as $key => $total)
                        <div class="col-md-3">
                            <strong>Total {{ $columns[$key] ?? $key }}:</strong> {{ $fmtMoney($total) }}
                        </div>
                    @endforeach
                </div>
            </div>
                ...
                <table class="table table-striped table-bordered align-middle">
                    ...
                    <tbody>
                        <tr class="table-warning fw-bold">
                            @foreach (array_keys($columns) as $key)
                                @if ($isCurrencyKey($key))
                                    <td class="text-end">{{ $fmtMoney($summaryTotals[$key] ?? 0) }}</td>
                                @elseif ($loop->first)
                                    <td>TOTAL</td>
                                @else
                                    <td>-</td>
                        ...
                        @forelse ($reports as $report)
                            <tr>
                                @foreach (array_keys($columns) as $key)
                                    ...
                                    @if ($key === 'Tranche')
                                        <td>{{ $value ?? '' }}</td>
                                    @elseif (str_contains(strtolower($key), 'date'))
                                        <td>{{ $fmtDate($value) }}</td>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('capital-report-form');
            if (!form) return;
