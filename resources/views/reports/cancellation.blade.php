...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center py-0">
            <h6 class="py-3 mb-0">Cancellation Report</h6>
        </div>
        <div class="card-body">
            <form method="get" action="{{ route('cmd.reports.cancellation_report') }}" class="bg-light rounded-3 p-2 p-md-3 mb-3" id="cancellation-form">
                ...
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="category" class="form-select form-select-sm">
                            ...
                            <option value="ldr" {{ ($filters['category'] ?? '') === 'ldr' ? 'selected' : '' }}>LDR</option>
                            <option value="plaw" {{ ($filters['category'] ?? '') === 'plaw' ? 'selected' : '' }}>ProLaw</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label mb-1">With Settlements Only</label>
                        <select name="with_settlements" class="form-select form-select-sm">
                            <option value="">All</option>
                            <option value="1" {{ ($filters['with_settlements'] ?? false) ? 'selected' : '' }}>Yes</option>
                            <option value="0" {{ ($filters['with_settlements'] ?? null) === '0' ? 'selected' : '' }}>No</option>
                        </select>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($reports as $report)
                                <tr>
                                    ...
                                    <td>
                                        @if($report->category === 'LDR')
                                            <span class="badge bg-primary">{{ $report->category }}</span>
                                        @elseif($report->category === 'ProLaw')
                                            <span class="badge bg-success">{{ $report->category }}</span>
                                        @elseif($report->category === 'PLAW')
