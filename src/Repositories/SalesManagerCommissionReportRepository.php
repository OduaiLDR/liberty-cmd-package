...
class SalesManagerCommissionReportRepository extends SqlSrvRepository
{
    /**
    ...
    protected function baseQuery(array $filters = []): Builder
    {
        ...
        $query = $this->table('TblEnrollment')
            ->select([
                ...
                DB::raw('ROUND(SUM(CASE WHEN Cancel_Date IS NULL AND NSF_Date IS NULL THEN Debt_Amount ELSE 0 END) * 0.0002, 2) AS commission'),
            ])
            ->whereNotNull('LLG_ID')
