...
use Carbon\Carbon;
use Cmd\Reports\Http\Requests\LlgExecAdminReportRequest;
use Cmd\Reports\Repositories\LlgExecAdminReportRepository;
...
class LlgExecAdminReportController extends Controller
{
    use CsvFormatting;

    public function __construct(
        protected LlgExecAdminReportRepository $repository
    ) {
    }

    public function index(LlgExecAdminReportRequest $request): View|StreamedResponse
    {
        ...
        $filters = $request->only(['client', 'source', 'employee_name', 'payment_type']);
        ...
        return view('reports::reports.llg_exec_admin', [
            'reports' => $reports,
    ...
    {
        $filename = 'llg_exec_admin_' . Carbon::now()->format('Ymd_His') . '.csv';
        $headers = [
