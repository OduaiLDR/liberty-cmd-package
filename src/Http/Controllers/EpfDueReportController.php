...
use Carbon\Carbon;
use Cmd\Reports\Http\Requests\EpfDueReportRequest;
use Cmd\Reports\Repositories\EpfDueReportRepository;
...
class EpfDueReportController extends Controller
{
    use CsvFormatting;

    public function __construct(
        protected EpfDueReportRepository $repository
    ) {
    }

    public function index(EpfDueReportRequest $request): View|StreamedResponse
    {
        ...
        return view('reports::reports.epf_due', [
            'reports' => $reports,
            'columns' => $columns,
            'filters' => $filters,
            'perPage' => $perPage,
    ...
    {
        $filename = 'epf_due_' . Carbon::now()->format('Ymd_His') . '.csv';
        $headers = [
