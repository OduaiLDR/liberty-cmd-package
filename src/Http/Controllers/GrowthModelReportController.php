...
use Carbon\Carbon;
use Cmd\Reports\Http\Requests\GrowthModelReportRequest;
use Cmd\Reports\Repositories\GrowthModelReportRepository;
...
class GrowthModelReportController extends Controller
{
    use CsvFormatting;

    public function __construct(
        protected GrowthModelReportRepository $repository
    ) {
    }

    public function index(GrowthModelReportRequest $request): View|StreamedResponse
    {
        ...
        $sample = $this->repository->sample($from, $to, 1000);
        $visibleColumns = $this->visibleColumns($sample, $allColumns);
        $summary = $this->summaries($sample, $visibleColumns);

        return view('reports::reports.growth_model', [
            'reports' => $reports,
    ...
    {
        $filename = 'growth_model_' . Carbon::now()->format('Ymd_His') . '.csv';
        $headers = [
    ...
    /**
     * @param  array<string, string>  $visibleColumns
     * @return array<string, array<string, float>>
     */
    protected function summaries(Collection $rows, array $visibleColumns): array
    {
        if ($rows->isEmpty()) {
            return [];
        }
