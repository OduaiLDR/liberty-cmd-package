...
use Carbon\Carbon;
use Cmd\Reports\Http\Requests\AgentRoiReportRequest;
use Cmd\Reports\Repositories\AgentRoiReportRepository;
...
class AgentRoiReportController extends Controller
{
    use CsvFormatting;

    public function __construct(
        protected AgentRoiReportRepository $repository
    ) {
    }

    public function index(AgentRoiReportRequest $request): View|StreamedResponse
    {
        ...
        $filters = $request->only(['agent', 'tier']);
        ...
        return view('reports::reports.agent_roi', [
            'reports' => $reports,
    ...
    {
        $filename = 'agent_roi_' . Carbon::now()->format('Ymd_His') . '.csv';
        $headers = [
    ...
    protected function summaries(Collection $rows): array
    {
        ...
        $keys = ['revenue_deals', 'cpa', 'revenue', 'commission', 'roi'];
