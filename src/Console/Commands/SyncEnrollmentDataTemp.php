...
class SyncEnrollmentDataTemp extends Command
{
    protected $signature = 'Sync:enrollment-data-temp';

    protected $description = 'Sync enrollment data temp: updates Drop_Name, State, Cancel_Date, and Payments from Snowflake and SQL Server sources';

    private string $source;

    public function handle(): int
    {
        $this->info("[INFO] Sync Enrollment Data Temp: starting for both LDR and PLAW.");
    ...
    {
        $this->source = $source;
        $this->info("[INFO] Sync Enrollment Data Temp: starting for {$this->source}.");
        ...
        try {
            ...
            Log::error('SyncEnrollmentDataTemp: Snowflake init failed', ['exception' => $e, 'source' => $this->source]);
            return Command::FAILURE;
        }
        ...
        try {
            ...
            Log::error('SyncEnrollmentDataTemp: SQL Server init failed', ['exception' => $e, 'source' => $this->source]);
            return Command::FAILURE;
        }
    ...
    private function updateContactsCampaign(DBConnector $sqlConnector): void
    {
        ...
        try {
            ...
            Log::warning('SyncEnrollmentDataTemp: Campaign update failed', [
                'source' => $this->source,
                'error' => $e->getMessage()
            ]);
        }
