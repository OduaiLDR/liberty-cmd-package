...
SELECT
    ...
    s.OFFER_ID,
    ROW_NUMBER() OVER(
        PARTITION BY t.CONTACT_ID, COALESCE(TO_VARCHAR(s.OFFER_ID), TO_VARCHAR(t.LINKED_TO), TO_VARCHAR(t.ID))
        ORDER BY t.DRAFT_DATE ASC
    ) AS N
FROM TRANSACTIONS AS t
LEFT JOIN SETTLEMENTS AS s
  ON t.LINKED_TO = s.TRANS_ID
 AND s._FIVETRAN_DELETED = 'FALSE'
WHERE t.TRANS_TYPE = 'PF'
  AND t.RETURNED_DATE IS NULL
  AND t._FIVETRAN_DELETED = 'FALSE'
ORDER BY t.CONTACT_ID ASC, COALESCE(s.OFFER_ID, t.LINKED_TO, t.ID) ASC, t.CLEARED_DATE ASC, t.DRAFT_DATE ASC
SQL;
...
WHERE t.TRANS_TYPE = 'PF'
  AND t.RETURNED_DATE IS NULL
  AND t._FIVETRAN_DELETED = 'FALSE'
  AND s.TRANS_ID IS NOT NULL
SQL,
            'pf_without_active_settlement' => <<<SQL
SELECT COUNT(*) AS CNT
FROM TRANSACTIONS AS t
LEFT JOIN SETTLEMENTS AS s
  ON t.LINKED_TO = s.TRANS_ID
