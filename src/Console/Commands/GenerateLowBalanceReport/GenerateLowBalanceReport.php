...
FROM (
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY b.CONTACT_ID ORDER BY CONVERT_TIMEZONE('America/Los_Angeles', b.STAMP) DESC) AS N
...
WHERE CAST(CONVERT_TIMEZONE('America/Los_Angeles', t.PROCESS_DATE) AS DATE) < '{$before}'
...
WHERE t.TRANS_TYPE = 'D'
  AND CAST(CONVERT_TIMEZONE('America/Los_Angeles', t.RETURNED_DATE) AS DATE) >= '{$sinceEsc}'
  AND c.ENROLLED = 1
  AND c.DROPPED = 0
  AND c.GRADUATED = 0
  AND c.DEL = 0
