...
latest AS (
    SELECT
        c.ID AS CONTACT_ID,
        cls.TITLE AS TITLE,
        CONVERT_TIMEZONE('America/Los_Angeles', cs.STAMP) AS STAMP,
        ROW_NUMBER() OVER (PARTITION BY cs.CONTACT_ID ORDER BY CONVERT_TIMEZONE('America/Los_Angeles', cs.STAMP) DESC, cs.ID DESC) AS rn
    FROM CONTACTS AS c
...
SELECT
    ...
    TITLE,
    TO_CHAR(CONVERT_TIMEZONE('America/Los_Angeles', STAMP), 'YYYY-MM-DD') AS STAMP
FROM latest
WHERE rn = 1
SQL;
